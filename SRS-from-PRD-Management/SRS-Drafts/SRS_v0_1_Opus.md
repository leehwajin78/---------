...
| ID | 제약 유형 | 내용 |
| :--- | :--- | :--- |
| CON-01 | 인프라 | Vercel Hobby 티어 (Serverless 실행 시간 ≤ 60초, 메모리 1024MB) |
| CON-02 | 인프라 | Supabase Free 티어 (500MB DB, 1GB Storage, 50K MAU) |
| CON-03 | 비용 | 월 운영비 하드캡 $100 (Claude API $100 도달 시 자동 중단) |
| CON-04 | 운영 | 1인 운영 모델 — 월 프로젝트 상한 6건 |
| CON-05 | AI | Claude API 200K 토큰 컨텍스트 윈도우 제약 |
| CON-06 | 보안 | OWASP Top 10 준수, API Key 클라이언트 노출 0% |
| CON-07 | 아키텍처 | ADR: Next.js + Vercel Serverless + Supabase 스택 고정 (MVP V1) |
...
| ASM-02 | Claude API 200K 토큰 컨텍스트 윈도우는 42문항 인터뷰 전체 텍스트 처리에 충분함 |
...
| **RLS** | Row Level Security — Supabase의 행 단위 접근 제어 정책 |
...
| :--- | :---: | :--- | :--- | :--- | :--- |
| `POST /api/diagnose` | POST | 5문항 진단 응답 (JSON) | 브랜드 지수·약점 리포트 (JSON) | Claude API, Supabase | F2, F4 |
| `POST /api/leads` | POST | 리드 정보 (이름, 연락처, 진단 결과) | 저장 확인 (JSON) | Supabase | F4 |
| `POST /api/generate-brief` | POST | 42문항 인터뷰 텍스트 (JSON) | 마스터 브리프 (JSON/Markdown, Streaming) | Claude API, Supabase | F1, F3 |
| `GET /api/projects/:id` | GET | 프로젝트 ID | 프로젝트 상태·브리프 데이터 (JSON) | Supabase | F3 |
| `PATCH /api/briefs/:id` | PATCH | 검수 수정 내용 (JSON) | 버전 업데이트 확인 (JSON) | Supabase | F3 |
| `GET /api/leads` | GET | 필터 파라미터 (상태, 날짜) | 리드 목록 (JSON) | Supabase | F4 |
| `POST /api/validate-diagnosis` | POST | 5문항 응답 패턴 (JSON) | 유효성 검증 결과 (JSON) | — | F2 |
| `PATCH /api/leads/:id/status` | PATCH | 리드 ID, 변경 상태 (JSON) | 상태 업데이트 확인 (JSON) | Supabase | F4 |
| `POST /api/briefs/:id/hallucination-tag` | POST | 환각 문장 ID, 태깅 데이터 (JSON) | 태깅 저장 확인 (JSON) | Supabase | F1, F3 |
| `GET /api/dashboard/metrics` | GET | 기간 필터 (월/분기) | KPI 집계 데이터 (JSON) | Supabase | F3, F4, F6 |
...
sequenceDiagram
    ...
    alt 불성실 응답 (전 문항 동일 선택)
        ...
        VAL-->>API: 유효 판정
        API->>Claude: System Prompt + 응답 데이터 전송
        Claude-->>API: 브랜드 지수·약점 리포트 반환
...
sequenceDiagram
    ...
    alt 20문항 미만
        ...
        VAL-->>API: 통과
        API->>Claude: System Prompt (168 패턴) + 인터뷰 텍스트
        Note over Claude: Streaming 출력 (최대 10분)
        Claude-->>API: 마스터 브리프 JSON/Markdown 반환
...
| UC-06 | AI 마스터 브리프 생성 | 운영자, Claude API | REQ-FUNC-001, 002, 004 |
...
| UC-12 | 녹취 텍스트 → B2B 카피 변환 | 운영자, Claude API | REQ-FUNC-014, 015 |
...
flowchart TB
    ...
    subgraph External ["External Services"]
        ...
        SUPA["Supabase\n(Database + Auth)\n\n- leads, diagnoses\n- clients, projects\n- briefs, retainers\n- RLS 적용"]
        GA4["Google Analytics 4\n(Event Tracking)\n\n- page_view\n- cta_click\n- report_scroll_complete"]
        SLACK["Slack Webhook\n(Alerting)\n\n- 오류 알림\n- 비용 알림\n- 리드 알림"]
        TF["Typeform\n(Survey)\n\n- NPS 설문\n- 월말 자가 보고"]
    end

    subgraph Infra ["Infrastructure"]
        VERCEL["Vercel\n(Hosting + CDN)\n\nHobby Tier\n- Edge CDN\n- Serverless 60s limit"]
    end
...
| 컴포넌트 | 기술 | 역할 | 제약 (CON) |
| :--- | :--- | :--- | :--- |
| **Client Layer** | Next.js (React) | SSR/SSG 기반 퍼블릭 페이지 + Admin SPA | CON-07 |
| **API Layer** | Vercel Serverless Functions | RESTful API, Streaming 지원 | CON-01 (실행 ≤ 60초) |
| **Validation Engine** | JavaScript (서버사이드) | 입력 유효성 검증, 불성실 패턴 탐지 | — |
| **Cost Monitor** | JavaScript (서버사이드) | Claude API 비용 추적 및 자동 차단 | CON-03 ($100 하드캡) |
| **Claude API** | Anthropic REST API | AI 텍스트 생성 (진단, 브리프, 카피) | CON-05 (200K 토큰) |
| **Supabase** | PostgreSQL + REST + Auth | 데이터 영속화, RLS 기반 접근 제어 | CON-02 (500MB) |
| **GA4** | gtag.js | 사용자 행동 이벤트 트래킹 | GDPR 동의 배너 |
| **Slack** | Incoming Webhook | 운영 알림 (오류, 비용, 리드) | — |
...
| **AI Service** | BriefGenerator | Claude API 호출, 프롬프트 관리, 입력 유효성 검증 |
...
| :--- | :--- | :--- | :---: | :--- |
| **REQ-FUNC-001** | 시스템은 42문항 인터뷰 텍스트를 입력받아 168개 프롬프트 패턴을 적용하여 15페이지 분량의 마스터 브리프 초안을 자동 생성해야 한다. | Story 1 (AC1), F1 | Must | **Given** 고객의 42문항 인터뷰 텍스트(20문항 이상 응답)가 입력된 상태 **When** 운영자가 관리자 콘솔에서 브리프 생성을 요청하면 **Then** 15p 분량의 마스터 브리프 초안이 30분 이내에 자동 생성된다. |
| **REQ-FUNC-002** | 시스템은 마스터 브리프 생성 시 Streaming 방식으로 결과를 출력하여 운영자가 실시간으로 생성 과정을 확인할 수 있어야 한다. | F1 | Must | **Given** 브리프 생성 요청이 Claude API로 전송된 상태 **When** AI가 응답을 생성하는 동안 **Then** 관리자 콘솔에 텍스트가 Streaming 방식으로 즉시 표시되며, 전체 생성 완료까지 빈 화면 대기 시간이 0초이다. |
| **REQ-FUNC-003** | 시스템은 인터뷰 응답이 20문항 미만인 경우 브리프 생성을 차단하고 "응답 부족 (최소 20문항 필요)" 경고를 출력해야 한다. | Story 1 (AC1-N) | Must | **Given** 입력된 인터뷰 텍스트가 20문항 미만인 상태 **When** 브리프 생성 요청이 제출되면 **Then** 시스템이 HTTP 400과 함께 "응답 부족 (최소 20문항 필요)" 메시지를 반환하고, Claude API 호출이 발생하지 않는다. 차단 정확도 100%. |
...
| **REQ-FUNC-005** | 시스템은 AI 도출 결과에 대한 고객 동의율이 50% 미만일 경우, 2차 보완 인터뷰 스케줄을 자동 생성하고 보완 질문 5문항을 사전 발송해야 한다. | Story 4 (AC1-N) | Must | **Given** 브리프 검수 미팅에서 고객 동의율이 50% 미만으로 기록된 상태 **When** 운영자가 동의율을 관리자 콘솔에 입력하면 **Then** 시스템이 1영업일 이내에 2차 보완 인터뷰(30분) 스케줄 생성 + 보완 질문 5문항을 자동 산출한다. |
...
| **REQ-FUNC-007** | 시스템은 생성된 브리프의 AI 환각(Hallucination) 문장에 대해 운영자 태깅 기능을 제공하여, 월간 환각율을 산출할 수 있어야 한다. | F1, 모니터링 | Must | **Given** 운영자가 브리프 검수 화면에서 특정 문장을 환각으로 태깅한 상태 **When** 월간 검수 데이터가 집계되면 **Then** 월간 환각율(환각 태깅 문장 수 / 전체 검수 문장 수 × 100)이 자동 산출되어 대시보드에 표시된다. |

#### 4.1.2 F2 — AI 브랜드 진단 툴 (Lead Funnel)

| ID | 요구사항 | Source | Priority | Acceptance Criteria |
| :--- | :--- | :--- | :---: | :--- |
| **REQ-FUNC-008** | 시스템은 잠재 고객에게 5문항 객관식 진단 폼을 제공하고, 응답 완료 시 Claude API를 통해 브랜드 지수(0~100점)와 약점 분석 리포트를 즉시 생성해야 한다. | Story 3 (AC1), F2 | Must | **Given** 잠재 고객이 랜딩페이지에서 5문항 진단 폼을 완료한 상태 **When** 폼이 제출되면 **Then** 10초 이내에 브랜드 지수(0~100점)와 약점 분석 리포트가 웹뷰에 출력된다. 폼 완료율 ≥ 60%. |
| **REQ-FUNC-009** | 시스템은 5문항 전체에서 동일 선택지를 고른 불성실 응답을 탐지하고, "응답 패턴 이상 — 다시 한번 확인해주세요" 메시지를 출력하며 리포트 생성을 차단해야 한다. | Story 3 (AC1-N), F2 | Must | **Given** 5문항 모두 동일 선택지가 선택된 상태 **When** 진단 폼이 제출되면 **Then** 시스템이 "응답 패턴 이상 — 다시 한번 확인해주세요" 메시지를 출력하고 Claude API 호출 및 리포트 생성을 차단한다. 불성실 필터 정확도 ≥ 95%, 정상 응답 오차단율 < 2%. |
...
| **REQ-FUNC-011** | 시스템은 CTA 클릭 이벤트를 GA4에 `cta_click`으로 전송하고, 리포트 완독 이벤트를 `report_scroll_complete`로 전송하여 전환율 측정을 지원해야 한다. | Story 3 (AC2), 보조 KPI 5 | Must | **Given** 고객이 리포트 페이지에서 상호작용하는 상태 **When** 리포트를 끝까지 스크롤하면 `report_scroll_complete` 이벤트가, CTA를 클릭하면 `cta_click` 이벤트가 **Then** GA4로 즉시 전송된다. 이벤트 전송 실패율 < 1%. |
...
| **REQ-FUNC-015** | 관리자 콘솔은 녹취 텍스트가 500자 이하이거나 핵심 키워드 추출이 불가능한 경우, "입력 부족 — 최소 1,000자 이상 필요" 경고를 출력하고 AI 생성을 차단해야 한다. | Story 2 (AC1-N) | Must | **Given** 입력된 녹취 텍스트가 500자 이하인 상태 **When** 변환 요청이 제출되면 **Then** 시스템이 "입력 부족 — 최소 1,000자 이상 필요" 경고를 출력하고 Claude API 호출을 차단한다. 경고 출력 정확도 100%. |
| **REQ-FUNC-016** | 관리자 콘솔은 비밀번호 + IP 화이트리스트 기반 접근 제어를 적용하여, 미등록 IP에서의 접속을 차단해야 한다. | NFR 보안 | Must | **Given** 미등록 IP에서 관리자 콘솔 접속을 시도한 상태 **When** 로그인 페이지에 접근하면 **Then** 접속이 즉시 차단되고, 차단 로그가 기록되며, Slack 알림이 즉시 발송된다. 차단율 100%. |
...
| **REQ-FUNC-018** | 관리자 콘솔은 검수 완료된 브리프에 대해 환각 문장 태깅 UI를 제공하여 운영자가 특정 문장을 클릭하여 "환각"으로 마킹할 수 있어야 한다. | F3, 모니터링 | Must | **Given** 브리프 검수 화면에서 운영자가 특정 문장을 선택한 상태 **When** "환각 태깅" 버튼을 클릭하면 **Then** 해당 문장이 환각으로 마킹되고, 태깅 데이터가 Supabase에 저장된다. |
...
| **REQ-FUNC-023** | 시스템은 리드 DB에서 월별 신규 리드 수를 `leads.created_at` 기반으로 자동 집계하여 관리자 대시보드에 표시해야 한다. | 보조 KPI 6, F4 | Must | **Given** 운영자가 관리자 대시보드에 접속한 상태 **When** 월간 리드 집계 섹션을 조회하면 **Then** 당월 신규 리드 수가 `leads.created_at` 월별 COUNT로 자동 산출되어 표시된다. |
...
| **REQ-FUNC-030** | 시스템은 Claude API 장애 시 "현재 AI 분석 서비스가 일시적으로 지연되고 있습니다. 4시간 내에 결과를 전달드리겠습니다." Fallback 메시지를 사용자에게 출력해야 한다. | NFR 신뢰성 | Must | **Given** Claude API 호출이 타임아웃 또는 5xx 에러를 반환한 상태 **When** 시스템이 에러를 감지하면 **Then** 사용자에게 Fallback 메시지가 출력되고, 운영자 Slack에 장애 알림이 발송된다. |
...
| :--- | :--- | :--- | :--- | :--- |
| **REQ-NF-001** | 진단 리포트 생성 응답 시간은 p95 기준 15초 이내여야 한다. | p95 ≤ 15초 (Claude API 호출 포함) | Vercel Functions Log | PRD §5-1, Story 3 (AC1) |
...
| **REQ-NF-008** | 데이터 백업은 Supabase 자동 백업을 통해 일 1회 수행되어야 한다. | 백업 주기: 1일 1회 | Supabase Dashboard | PRD §5-2 |
| **REQ-NF-009** | Claude API 장애 시 수동 Fallback 프로세스가 4시간 이내에 완료되어야 한다. | Fallback SLA ≤ 4시간 | Slack 알림 타임스탬프, 수동 처리 로그 | PRD §5-2 |
...
| **REQ-NF-013** | 관리자 콘솔은 비밀번호 + IP 화이트리스트 기반 접근 제어를 적용해야 한다. | 미등록 IP 접속 차단율 100%, 차단 로그 즉시 알림 | Supabase Auth 로그, Slack 알림 | PRD §5-3 |
...
| ID | 요구사항 | 측정 기준 | 측정 도구 | Source |
| :--- | :--- | :--- | :--- | :--- |
| **REQ-NF-015** | 월간 총 운영비는 $100(하드캡) 이내여야 한다. | 월 비용 ≤ $100 | Anthropic Dashboard, Vercel Dashboard | PRD §5-4 |
| **REQ-NF-016** | Claude API 비용이 월 $100에 도달하면 API 호출을 자동 중단하고, 대표 수동 승인 후에만 재개해야 한다. | $100 도달 시 자동 중단 | Anthropic Dashboard 연동, Slack 긴급 알림 | PRD §5-4 |
...
| **REQ-NF-022** | Claude API 대체 LLM(GPT-4o, Gemini)으로의 전환이 72시간 이내에 완료 가능하도록, AI 엔진과 비즈니스 로직을 분리하여 구현해야 한다. | LLM 전환 소요시간 ≤ 72시간 | 리스크 R4 |
...
| :---: | :--- | :---: | :--- | :--- | :--- | :--- |
| API-01 | `POST /api/diagnose` | POST | 5문항 진단 응답 → Claude API → 브랜드 지수·리포트 생성 + 리드 저장 | 없음 (Public) | 50 req/hr per IP | REQ-FUNC-008, 009 |
| API-02 | `POST /api/leads` | POST | 상담 신청 리드 정보 저장 | 없음 (Public) | 50 req/hr per IP | REQ-FUNC-019, 020 |
| API-03 | `POST /api/generate-brief` | POST | 42문항 인터뷰 텍스트 → Claude API → 마스터 브리프 생성 (Streaming) | Admin (비밀번호 + IP 화이트리스트) | 10 req/hr | REQ-FUNC-001, 002, 003 |
...
#### 6.2.7 ERD (Entity Relationship Diagram)
...
sequenceDiagram
    ...
    alt 불성실 응답 (전 문항 동일)
        ...
        VAL-->>API: 유효 판정
        API->>Claude: System Prompt + 응답 데이터
        Note over Claude: 처리 시간 p95 15초 이내
        Claude-->>API: 브랜드 지수 + 약점 리포트
...
sequenceDiagram
    ...
    alt 20문항 미만
        ...
        alt 500자 이하
            ...
            VAL-->>API: 통과
            API->>Claude: System Prompt (168 패턴) + 인터뷰 텍스트
            Note over Claude: Streaming (p95 10분 이내)
            Claude-->>API: 마스터 브리프 반환
...
sequenceDiagram
    participant API as Vercel API
    participant Monitor as 비용 모니터
    participant Anthropic as Anthropic Dashboard
    participant Slack as Slack
    actor 대표 as 운영자 (대표)

    loop 매 API 호출 시
        API->>Monitor: 호출 비용 누적
        Monitor->>Anthropic: 일일/월간 비용 확인
        alt 일일 비용 > $5
            Monitor->>Slack: 일일 비용 $5 초과 알림
            Slack-->>대표: 이메일 알림 전달
        end
        alt 월간 비용 >= $100
            Monitor->>API: API 호출 자동 중단
            Monitor->>Slack: 월 $100 하드캡 도달 - API 중단
            Slack-->>대표: 긴급 알림
            대표->>Monitor: 수동 승인 (재개 요청)
...
> | 데이터 엔터티 | 6개 (LEAD, DIAGNOSIS, CLIENT, PROJECT, BRIEF, RETAINER) |
